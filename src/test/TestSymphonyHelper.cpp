...
TEST_CASE("Test doBranchAndBoundWithSymphony", "[SymphonyHelper::doBranchAndBoundWithSymphony]") {
  ...
  vpc_params.set(BB_STRATEGY, get_bb_option_value({
      ...
      BB_Strategy_Options::presolve_off,  // don't use presolve since it can't pass cuts through
  ...
  SECTION( "Test first instance solve" ) {
    ...
    // solve with symphony
    BBInfo info;
    std::shared_ptr<CoinWarmStart> ws = doBranchAndBoundWithSymphony(
        vpc_params, vpc_params.get(BB_STRATEGY), solver, info, &vpcs, nullptr, nullptr);
  ...
  SECTION( "Test objective perturbed warm-start" ) {
    ...
    BBInfo info_initial;
    std::shared_ptr<CoinWarmStart> ws = doBranchAndBoundWithSymphony(
        vpc_params, vpc_params.get(BB_STRATEGY), solver, info_initial, &vpcs);

    // create a solver with a perturbed objective but same solution
    ...
    doBranchAndBoundWithSymphony(vpc_params, vpc_params.get(BB_STRATEGY), solver,
                                 info_ws, &vpcs, ws.get(), si_ptb, &vpcs);
